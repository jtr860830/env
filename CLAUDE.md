...
**Light `comment` is deliberately *not* Atom's mono-2, and the grey ramp is therefore two steps rather than three.** Atom's greyscale is mono-1/2/3, and the dark half lands on it (`fg` `#abb2bf`, `comment` `#7f848e` ≈ `#828997`, `gray` `#5c6370`); `onelight` keeps onedarkpro's own `#9b9fa6`, which sits **1.03** from `gray` `#a0a1a7` — the same colour. Substituting mono-2 `#696c77` restores an even ramp (fg↔comment 2.17, comment↔gray 2.03, against the dark half's 1.76 / 1.61) and takes `comment` from 2.55 to 5.01 against `bg`. Declined by eye: the dim comment reads comfortably, and doubling its contrast would make comments compete with code. The collapse is visible in exactly one place — Alfred's result row puts `subtext` (comment) next to `shortcut` (gray), so in light they are indistinguishable, and selecting a row moves the shortcut from 2.24 to **1.64** instead of rising as it does in dark (2.55 → 2.88). Accepted for the same reason. Re-measuring this ramp and finding 1.03 is not a new discovery.
